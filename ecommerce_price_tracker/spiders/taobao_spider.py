...
class TaobaoSpider(BaseSpider):
    ...
    def parse(self, html: str) -> Dict[str, Any]:
        ...
        try:
            # 商品名称
            name = ''
            name_elem = soup.find('h1', class_='tb-main-title')
            if name_elem:
            ...
            # 原价
            original_price = 0.0
            original_price_elem = soup.find('span', class_='tb-rmb-exp')
            ...
            # 品牌
            brand = ''
            brand_elem = soup.find('div', class_='tb-property')
            if brand_elem:
            ...
            # 商品图片
            image_url = ''
            image_elem = soup.find('img', id='J_ImgBooth')
            if image_elem and 'src' in image_elem.attrs:
                image_url = image_elem['src']
            elif image_elem and 'data-src' in image_elem.attrs:
                image_url = image_elem['data-src']
            
            # 库存状态
            stock_status = 'in_stock'
            stock_elem = soup.find('div', class_='tb-btn-buy')
            if stock_elem:
                if '无货' in str(stock_elem) or '已下架' in str(stock_elem):
